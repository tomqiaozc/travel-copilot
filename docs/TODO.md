...
- 已实现：Docker PostgreSQL 16 持久化存储（`docker-compose.yml`），设置 `DATABASE_URL` 即可启用；内存模式仍保留作为备选
...
### ~~5. 拖拽体验~~ ✅ 已完成
- ~~跨天拖拽后需重新排序 order_in_day（当前只更新被拖拽的地点，不调整同天其他地点顺序）~~ ✅ 已实现批量重排端点 `PUT /reorder`，拖拽后所有受影响天的地点自动重新编号
- ~~拖拽时的视觉反馈（当前较基础）~~ ✅ 已实现拖拽卡片阴影/缩放 + 放置区域蓝色高亮
...
- ~~可进一步优化：进度条、骨架屏、预估时间等~~ ✅ 已实现分步进度指示器（StepProgress）和全页面骨架屏（Skeleton）
